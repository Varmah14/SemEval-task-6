...
def train_model(task='clarity', model_name='google/electra-base-discriminator'):    
    print(f"\n{'='*70}")
    print(f"Training ELECTRA-base (NO FEATURES): {task.upper()} task")
    print(f"{'='*70}")
    ...
    model=ELECTRAClassifier(model_name, num_labels, class_weights=class_weights).to(device)
    
    training_args=TrainingArguments(
        output_dir=f"../results/{task}_electra_no_features",
        num_train_epochs=5,
    ...
    df_report.to_csv(f"../csv_files/{task}_electra_no_features_results.csv")
